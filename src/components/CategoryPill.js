...
function CategoryPill({ category, user,  setIsSelected , isSelected}) {


        const select = () => {
            axios.post(`${API}/users/${user?.id}/category/${category.id}`)
            .then(() => {
    ...
    const unselect = () => {
        axios.delete(`${API}/users/${user?.id}/category/${category.id}`)
        .then(() => {
            axios.get(`${API}/users/${user?.id}/category`)
            .then((res) => {
                setIsSelected(res.data)
    ...
    let userCategory = isSelected.map((ele) => {
        return ele.name
    })
