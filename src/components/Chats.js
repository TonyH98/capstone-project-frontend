...
function Chats({ loggedin, setLoggedin, user, setUser, firebaseId }) {
  ...
  const onLoadMessageList = (user_id) => {
    ...
    axios
      .post(`${API}/chats?sender_id${user_id}&receiver_id=${Global.user?.id}`)
      .then((res) => {
          ...
          {searchUser ? (
            <div className="flex gap-4">
              <div className="bg-cyan-100/50 rounded-md p-4 overflow-y-auto min-h-screen">
                <h2 className="text-2xl text-cyan-400 font-semibold py-2 px-4">
                  Searched User
                </h2>
                {filterUsers.map((recipient) => (
                  <div
                    ...
                    className={`font-semibold py-1 flex gap-1 ${
                      selectedUser && selectedUser.id === recipient.id
                        ? "bg-cyan-400"
                        : ""
                    }`}
                  >
                    ...
                    <span>
                      {recipient.first_name[0].toUpperCase() +
                        recipient.first_name.substring(1)}{" "}
                      {recipient.last_name[0].toUpperCase() +
                        recipient.last_name.substring(1)}
                    </span>
                  </div>
                ))}
              </div>
                ...
                {users.map((recipient, recipientKey) =>
                  Global.user?.id === recipient.id ? null : (
                    <div
                      ...
                      className={`${
                        active ? "bg-cyan-400" : ""
                      } font-semibold py-1 px-4 flex gap-1`}
                      style={{
                        cursor: "hand",
                        color:
                          selectedUser?.id === recipient.id ? "red" : "black",
                      }}
                    >
                      <span className="hidden">
                        {(receiver = JSON.stringify(recipient))}
                      </span>
                      {/* <span><img src={recipient.profile_img} alt="" className="w-10 rounded-full"></img></span> */}
                      <span>
                        {recipient.first_name[0].toUpperCase() +
        ...
        <div className="flex-auto p-4 overflow-y-auto min-h-screen">
          {!Global.user ? (
            <div className="text-center">
              <div>
                Log In or Create an account to start a conversation with other
                users!
              </div>
            </div>
          ) : (
            <div className="flex gap-6 px-4 flex-auto ml-20 min-h-screen bg-cyan-100/50 rounded-md md:w-[450px] lg:w-[700px]">
              <div className="flex flex-col p-4">
                    ...
                    return (
                      <div
                        className={`my-1 flex flex-col ${
                          val.author === user.id ? "items-end" : "items-start"
                        }`}
                        key={key}
                      >
                        <div
                ...
                <div className="p-4 sticky flex">
                  ...
                  />
                  <button onClick={sendMessage} className="hover:text-cyan-400">
                    <BiSend size={35} />
                  </button>
                </div>
              </div>
