...
//   // function to create a new account with firebase and update the newUser object with firebase_id
  const createUserCredentials = async () => {
    try {
        const userCredentialResponse = await createUserWithEmailAndPassword(auth, newUser.email, newUser.password)
...
const checkAge = () => {
  ...
  useEffect(() => {
    let age = Number(prompt("Please note this site is 18+ only. Enter your age in years below:"));
